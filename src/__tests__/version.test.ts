import { Version, requiresVersion } from "../version";
...
describe("requiresVersion", () => {
	...
	it("throws older version", () => {
		...
		expect(() => requiresVersion(99, appVersion, "Test")).toThrow(
			`[ERR_NOT_SUPPORTED]: Test requires Stream Deck version 99.0 or higher, but current version is 1.0; please update Stream Deck and set the "Software.MinimumVersion" in the plugin's manifest to "99.0" or higher.`
		);
	});
});

/**
 * Provides assertions for {@link Version}.
 */
describe("Version", () => {
	/**
	...
	describe("serialization", () => {
		...
		describe("valid", () => {
			...
			it.each(cases)("stringifies $name", ({ value }) => {
				// Arrange, act, assert.
				const version = new Version(value);
				expect(`${version}`).toBe(`${version.major}.${version.minor}.${version.patch}.${version.build}`);
			});
		});
