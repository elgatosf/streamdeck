...
import { DeviceStore, deviceStore } from "./store";

/**
 * Provides functions, and information, for interacting with Stream Deck actions.
 */
class DeviceService extends DeviceStore {
	/**
	...
	public onDeviceDidConnect(listener: (ev: DeviceDidConnectEvent) => void): IDisposable {
		return connection.disposableOn("deviceDidConnect", (ev) => listener(new DeviceEvent(ev, deviceStore.getDeviceById(ev.device)!)));
	}
	...
	public onDeviceDidDisconnect(listener: (ev: DeviceDidDisconnectEvent) => void): IDisposable {
		return connection.disposableOn("deviceDidDisconnect", (ev) => listener(new DeviceEvent(ev, deviceStore.getDeviceById(ev.device)!)));
	}
}
 ...
 * Provides functions, and information, for interacting with Stream Deck actions.
 */
export const devices = new DeviceService();
