...
// (vscode-streamdeck: src/debug/pipe-path.ts). Both sides compute the rendezvous path from the
// plugin UUID independently; if they diverge, the extension can no longer find the plugin.

/**
 * Computes the debug transport pipe path for a plugin, derived deterministically from its UUID.
 *
 ...
 */
export function getDebugPipePath(uuid: string): string {
